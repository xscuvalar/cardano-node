...
queryPParamsFromLocalState network socketPath point = do
  let pointAndQuery = (point, QueryShelley GetCurrentPParams)
...
queryStakeDistributionFromLocalState network socketPath point = do
  let pointAndQuery = (point, QueryShelley GetStakeDistribution)
  res <- newExceptT $ liftIO $
    queryNodeLocalState
      nullTracer
      (pClientInfoCodecConfig . protocolClientInfo $ nodeClientProtocolCardano)
  ...
  lbs <- fmap (unSerialised . getRes) <$>
            newExceptT . liftIO $
              queryNodeLocalState
                ...
                network
                socketPath
                (point, QueryShelley (GetCBOR GetCurrentEpochState)) -- Get CBOR-in-CBOR version
...
queryDelegationsAndRewardsFromLocalState network socketPath addrs point = do
    ...
    let pointAndQuery = (point, QueryShelley (GetFilteredDelegationsAndRewardAccounts creds))
    res <- newExceptT $ liftIO $
      queryNodeLocalState
        nullTracer
        (pClientInfoCodecConfig . protocolClientInfo $ nodeClientProtocolCardano)
...
applyUTxOFilter qFilter =
    case qFilter of
      FilterByAddress addrs -> do shelleyAddrs <- checkAddresses $ partitionAddresses addrs
                                  Right $ QueryShelley $ GetFilteredUTxO shelleyAddrs
      NoFilter -> Right $ QueryShelley GetUTxO
  where
